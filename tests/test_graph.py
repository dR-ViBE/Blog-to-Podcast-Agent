...
class TestGraphCompilation:
    ...
    def test_graph_nodes_exist(self):
        ...
        assert not missing_nodes, (
            f"Graph is missing expected nodes: {missing_nodes}. Found: {node_names}"
        )
...
class TestDecideNextStep:
    ...
    def test_accepted_script_routes_to_audio(self):
        ...
        assert result == GENERATE_AUDIO, f"Expected GENERATE_AUDIO when accepted, got: {result}"
    ...
    def test_rejected_at_limit_routes_to_end(self):
        ...
        result = decide_next_step(state)
        assert result == END, f"Expected END when rejected at max retries, got: {result}"
